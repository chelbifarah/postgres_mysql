...
def affichage_parametre_serveur():
    ...
    if choix == '1':
        ...
        print("Type de la base de données: SQL Server")
        print("Base de données: ",db_name)
        #if you ever change tha database, you need the change it in the function: serveur_sqlserver()
...
def pandas_columns_mysql():
    global user_name, password_user
...
def insert_line_database(row, header_query):
    ...
    sql = "insert into {} ({}) " \
          "values " \
          "{};".format(table_name, header_query, ligne)
    return sql
...
def add_one_column(table_name, col):
    sql = "alter table {} add {} varchar(255)".format(table_name, col)
    return sql
...
if __name__ == '__main__':
    ...
    while choix != '1' and choix != '2' and choix != '3':
        choix = input("Voulez vous utiliser 1: MySQL ,  2: PostgreSQL ou  3: SQL server?  ")
        if choix != '1' and choix != '2' and choix != '3':
            print("j'ai pas compris! réessayez SVP et choisissez seulement 1 , 2  ou  3\n")
    ...
    if choix == '1':
        ...
        user_name = input("Entrez le nom d'utilisateur: ")
        password_user = input("Entrez le mot de passe: ")
        cnx = serveur_postgresql()
        print("connected")
    else:
        cnx = serveur_sqlserver()
        print("connected")
    cursor = cnx.cursor()
    ...
    try:
        ...
        #la requete de creation de table
        if choix == '1' or choix == '2':
            sql = "CREATE TABLE if not exists {} ({});".format(table_name, var_head)
        else:
            sql = "if not exists (select * from sysobjects where name= '{}' and xtype='U')"\
                  " create table {} ({}) " .format(table_name, table_name, var_head)
    ...
    try:
        ...
        columns_not_exist = []
        for i in range(n):
            if header_tuple[i].lower() not in columns_lower:
        ...
        print("Couldn't read the database columns")
    ...
    try:
        ...
        if len(columns_not_exist) != 0:
            ...
            while choice_add_columns.lower() != "oui" and choice_add_columns.lower() != "non":
                choice_add_columns = input(
                    "Êtes vous sûr(e) de vouloir ajouter les colonnes suivantes {} dans la table {}? répondez par 'oui' ou 'non' SVP! ".format(
                        columns_not_exist, table_name))
            ...
            if choice_add_columns.lower() == "oui":
                ...
                for i in range(nb_col_not_exist):
                    if nb_col_not_exist == 1:
                        col = columns_not_exist[0]
                        query_add_col = add_one_column(table_name, col)
                ...
                for row in rows:
                    ...
                    sql_query = insert_line_database(row, header_query)
                ...
                while choice_import_info.lower().strip() != "non" and choice_import_info.lower().strip() != "oui":
                    choice_import_info = input(
                        "Voulez-vous quand même importer les données sans ajouter les colonnes {} ?  ".format(
                            columns_not_exist))
                ...
                if choice_import_info.lower() == "non":
                    ...
                    for row in rows:
                        ...
                        sql_query = insert_line_database(new_row, list_columns_db)
